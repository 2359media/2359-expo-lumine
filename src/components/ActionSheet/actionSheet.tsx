...
import {AlertButton, SafeAreaView} from 'react-native';
...
export function showActionSheet(props: ActionSheetProps) {
  ...
  showModal(
    d => (
      <SafeAreaView>
        {props.title && (
          <ListItem
            style={{marginHorizontal: 16, flexDirection: 'column'}}
            type="action"
            isFirst
            rounded
          >
            ...
            {props.subtitle && (
              <Text p4 style={{textAlign: 'center'}}>
                {props.subtitle}
              </Text>
            )}
          </ListItem>
        ...
        {otherButtons.map((b, i) => (
          <ListItem
            ...
            type={b.style ?? 'action'}
            isFirst={i == 0 && !props.title}
            isLast={i == otherButtons.length - 1}
            rounded
          />
        ))}
        <ListItem
