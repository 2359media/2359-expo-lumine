...
const AnimatedKASV = r.Animated.createAnimatedComponent(KeyboardAwareScrollView);
...
export const SFAScrollView = forwardRef((props, ref) => {
    ...
    const pulldownStyle = r.useAnimatedStyle(() => ({
        transform: [
            {
                translateY: r.interpolate(offset.value, [-1, 0, 1, 2].map(x => h.value + x), [-60, -60, 0, 0]),
            },
        ],
        opacity: r.interpolate(offset.value, [-1, 0, 50, 51].map(x => h.value + x), [0, 0, 1, 1]),
    }));
    ...
    const scrollHandler = r.useAnimatedScrollHandler(e => {
        ...
        onScroll && r.runOnJS(onScroll)({ nativeEvent: e });
    });
    return (<View style={styles.container}>
      ...
      <Comp ref={r => {
            ...
            ios: refreshControl && (<r.Animated.View style={progressStyle}>
              {refreshControl}
            </r.Animated.View>),
            android: refreshControl,
        })}>
        <r.Animated.View style={topNavStyle} onLayout={e => (h.value = e.nativeEvent.layout.height)}>
          {topHero}
        </r.Animated.View>
