...
import { NavigationContainer, createNativeStackNavigator, getScreen, } from './services/navigation';
...
export { addProduct } from './services/product';
export default function App() {
    const Stack = useMemo(createNativeStackNavigator, []);
    const themeIndex = useSelector(s => s.settings.themeIndex);
    return (<ThemeContext.Provider value={themeIndex ? darkTheme : defaultTheme}>
      <AppProvider asyncs={[loadStore]}>
        <StatusBar barStyle={themeIndex ? 'light-content' : 'dark-content'} translucent backgroundColor="transparent"/>
        <NavigationContainer>
          <Stack.Navigator screenOptions={{ headerShown: false }}>
            ...
            <Stack.Screen name="ProductDetails" component={getScreen('ProductDetails')}/>
          </Stack.Navigator>
        </NavigationContainer>
      </AppProvider>
